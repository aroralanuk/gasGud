...
export default function Home() {
  ...
  const [callbackAddr, setCallbackAddr] = useState<string>("0xDD1156315Fc4cCC942d5f8303F9A28c92Cf0B2fe");
  const [functionSignature, setFunctionSignature] = useState<string>("axiomV2Callback(uint64,address,bytes32,bytes32,bytes32[],bytes)");
  ...
  useEffect(() => {
    const setupWorker = async () => {
      ...
      workerApi.current = await new Halo2Circuit(process.env.NEXT_PUBLIC_PROVIDER_URI as string);
  ...
  const generateAndSendQuery = async () => {
    ...
    let callbackFunctionSelector = (functionSignature && functionSignature.length > 0) ? keccak256(toHex(functionSignature)).slice(0, 10) : "0x00000000";
  ...
  return (
    <div className="flex flex-col p-4 gap-2">
      ...
      <div className="flex flex-col gap-2">
        <div className="flex flex-col">
          <label htmlFor="callbackAddr" className="mr-2">Callback Address:</label>
          <textarea id="callbackAddr" value={callbackAddr} onChange={(e) => setCallbackAddr(e.target.value)} className="px-1 border rounded" />
        </div>
        <div className="flex flex-col">
          <label htmlFor="callbackFunctionSelector" className="mr-2">Callback Function Selector:</label>
          <textarea id="callbackFunctionSelector" value={functionSignature} onChange={(e) => setFunctionSignature(e.target.value)} className="px-1 border rounded" />
        </div>
        <div className="flex flex-col">
          <label htmlFor="callbackExtraData" className="mr-2">Callback Extra Data:</label>
          <textarea id="callbackExtraData" value={callbackExtraData} onChange={(e) => setCallbackExtraData(e.target.value)} className="px-1 border rounded" />
        </div>
      </div>
